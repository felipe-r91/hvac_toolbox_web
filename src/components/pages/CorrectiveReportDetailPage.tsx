...
export function CorrectiveReportDetailPage() {
  ...
  if (error || !report) return <CardText text={error || "Corrective report not found."} error />;
    ...
    <section className="space-y-4">
      <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900">Corrective Report</h1>
          ...
          <div className="flex gap-2">
            ...
            <span className={`rounded-full px-3 py-1 text-xs font-medium ring-1 ${returnedStatusClasses(report.machineReturnedToService)}`}>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <h2 className="text-lg font-semibold text-slate-900">Failure Classification</h2>
        ...
        {report.photos.length > 0 ? (
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
            {report.photos.map((photo) => (
              <div key={photo.id} className="rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-200">
                <img
                  src={resolvePhotoUrl(photo.previewUrl)}
                  alt={photo.caption || "Corrective photo"}
                  className="h-56 w-full rounded-2xl object-cover"
                />
                <p className="mt-3 text-sm text-slate-700">{photo.caption || "No caption"}</p>
