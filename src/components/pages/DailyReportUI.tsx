...
export type AiDailyReport = {
    ...
    workConducted?: string[] | string;
    workConductedToday?: string[] | string;
    furtherActions?: string[] | string;
    ehsStatement?: string;
};

export type SourceDailyReport = {
    id?: string;
    vesselName?: string;
    vesselId?: string;
...
function buildInitialPages(includeAlarms: boolean): ReportPage[] {
    return [
        { id: "page-1", sections: ["vessel", "service"] },
        { id: "page-2", sections: includeAlarms ? ["equipment", "alarms"] : ["equipment"] },
        { id: "page-3", sections: ["workConducted"] },
        { id: "page-4", sections: ["photos"] },
        { id: "page-5", sections: ["recommendations", "ehs"] },
    ];
}
...
function AlarmCard({
    ...
    const solved = alarm.status?.toLowerCase() === "solved";
    ...
    return (
        <li className="group flex gap-3 bg-amber-50 p-2.5 text-sm text-amber-950 ring-1 ring-amber-100">
            ...
            <EditableText className="flex-1">{alarm.description}</EditableText>

            <button
                type="button"
                onClick={isEditing ? onToggleSolved : undefined}
                className={`shrink-0 border px-2 py-1 text-[10px] font-bold uppercase tracking-wide print:pointer-events-none ${solved
            ...
            <div className="border-t border-slate-300 bg-white px-4 py-3">
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-slate-500">
                        <EditableText>Daily Report Status</EditableText>
                    </p>
...
function SignatureBlock({ report }: { report: NormalizedDailyReport }) {
    return (
...
export default function DailyReportUI({
    aiReport,
    sourceReport,
}: {
    ...
    const report = React.useMemo<NormalizedDailyReport>(() => ({
        reportNo: aiReport.reportNo || `DR-${sourceReport.id || "DRAFT"}`,
        title: aiReport.title || "Daily Report",
        subtitle:
            aiReport.subtitle ||
        ...
        alarms:
            aiReport.alarms && aiReport.alarms.length > 0
                ? aiReport.alarms.map((alarm) => ({
                    description: alarm.description || "Alarm / abnormal reading",
                    status: alarm.status || "",
                }))
                : sourceReport.alarmPresent
                ? [
                    {
                        description:
                            [
                                sourceReport.failureCode,
                                sourceReport.failureComponent,
                                sourceReport.failureMode,
                            ]
                                .filter(Boolean)
                                .join(" - ") ||
                            sourceReport.failureNotes ||
                            "Alarm / abnormal reading",
                        status: "Active",
                    },
                ]
                : [],
        hasActiveAlarm:
            Boolean(sourceReport.alarmPresent) ||
            (aiReport.alarms || []).some((alarm) => alarm.status?.toLowerCase() !== "solved"),
        workConducted: normalizeList(aiReport.workConducted || aiReport.workConductedToday || sourceReport.workConductedToday),
        furtherActions: normalizeList(aiReport.furtherActions || sourceReport.furtherActions),
        ehsStatement:
    ...
    const [alarms, setAlarms] = React.useState<AlarmItem[]>(report.alarms);
    const [workConducted, setWorkConducted] = React.useState<string[]>(report.workConducted);
    const [recommendations, setRecommendations] = React.useState<string[]>(report.furtherActions);
    const [pages, setPages] = React.useState<ReportPage[]>(buildInitialPages(report.hasActiveAlarm));
    ...
    React.useEffect(() => {
        ...
        setRecommendations(report.furtherActions);
        setPages(buildInitialPages(report.hasActiveAlarm));
    ...
    function renderSection(sectionId: SectionId) {
        switch (sectionId) {
            ...
            case "alarms":
                if (!report.hasActiveAlarm) return null;
                            ...
                            isEditing ? (
                                <button
                                    ...
                                    onClick={() =>
                                        setAlarms((current) => [
                                            ...current,
                                            { description: "New alarm / abnormal reading", status: "" },
                                        ])
                                    }
                ...
                return (
                    ...
                    >
                        {firstWorkConductedItems.length > 0 ? (
                            ...
                            <p className="text-sm text-slate-500">No work conducted provided.</p>
                        )}
                    </Section>
                );
                ...
                return (
                        ...
                        right={
                            isEditing ? (
                                <button
                                    type="button"
                                    onClick={() =>
                                        setRecommendations((current) => [...current, "New further action"])
                                    }
                                    className="mr-24 border border-[#003594] bg-white px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-[#003594] hover:bg-[#EAF6FB] print:hidden"
                                >
                                    Add action
                    ...
                    >
                        {firstRecommendationItems.length > 0 ? (
                            ...
                            <p className="text-sm text-slate-500">No further actions provided.</p>
                        )}
                    </Section>
                );
            case "recommendationsContd":
                return (
                    <Section icon={FaCheckCircle} title="Further Actions Contd.">
                        {continuedRecommendationItems.length > 0 ? (
                            ...
                            <p className="text-sm text-slate-500">No continued further actions.</p>
                        )}
                    </Section>
                );
    ...
    async function handleUploadCustomerReport() {
        ...
        try {
            ...
            const filename = `${report.reportNo || "daily-report"}.pdf`.replaceAll(
                /[^a-zA-Z0-9._-]/g,
                "_"
            );
                ...
                >
                    {pages.map((page, pageIndex) => (
                        ...
                        >
                            ...
                            >
                                {pageIndex === 0 && (
                                    <ReportHeader report={report} isPrintPreview={isPrintPreview} hasActiveAlarm={hasActiveAlarm} />
                                )}
