...
export type SourceServiceReport = {
    id?: string;
    vesselName?: string;
    vesselId?: string;
...
const initialPages: ReportPage[] = [
    ...
    { id: "page-2", sections: ["equipment", "summary", "alarms"] },
    { id: "page-3", sections: ["photos"] },
    { id: "page-4", sections: ["workConducted", "recommendations"] },
...
function ReportHeader({
    ...
    report: NormalizedServiceReport;
    isPrintPreview: boolean;
    hasActiveAlarm: boolean;
}) {
            ...
            <div className="border-t border-slate-300 bg-white px-4 py-3">
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-slate-500">
                        <EditableText>Service Result</EditableText>
                    </p>
...
function SignatureBlock({ report }: { report: NormalizedServiceReport }) {
    return (
...
export default function ServiceReportUI({
    aiReport,
    sourceReport,
}: {
    ...
    const report: NormalizedServiceReport = {
        reportNo: aiReport.reportNo || `SR-${sourceReport.id || "DRAFT"}`,
        title: aiReport.title || "Service Report",
        subtitle:
            aiReport.subtitle ||
        ...
        hasActiveAlarm:
            (aiReport.alarms || []).some(
                (alarm) => alarm.status?.toLowerCase() !== "solved"
            ),
        workConducted: aiReport.workConducted || [],
    ...
    const [recommendations, setRecommendations] = React.useState<string[]>(report.recommendations);
    ...
    async function handleUploadCustomerReport() {
        ...
        try {
            ...
            const filename = `${report.reportNo || "service-report"}.pdf`.replaceAll(
                /[^a-zA-Z0-9._-]/g,
                "_"
            );
            ...
            await uploadCustomerReportPdf(
                {
                    ...
                    sourceReportType: "service_report",
